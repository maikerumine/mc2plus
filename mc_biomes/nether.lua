...
minetest.register_node("mc_biomes:portal", {
...
local function build_portal(pos, target)
	...
	for y = p1.y, p2.y do
		...
		if not (x == p1.x or x == p2.x or y == p1.y or y == p2.y) then
			minetest.set_node(p, {name = "mc_biomes:portal", param2 = 0})
		end
...
function make_portal(pos)
	...
	for y = p1.y + 1, p2.y - 1 do
		...
		--or minetest.get_node(p).name ~= "mc_biomes:void" 
		--or minetest.get_node(p).name ~= "mc_biomes:nether_air" 
		then
			return false
		end
		--[[
		...
		if minetest.get_node(p).name ~= "mc_biomes:void" 
		then
			return false
		end
		
		if minetest.get_node(p).name ~= "mc_biomes:nether_air" 
		then
			return false
		end]]
	end
	...
	for y = p1.y, p2.y do
		...
		--or minetest.get_node(p).name == "mc_biomes:nether_air" 
		then
			minetest.set_node(p, {name = "mc_biomes:portal", param2 = param2})
		end
		local meta = minetest.get_meta(p)
		meta:set_string("p1", minetest.pos_to_string(p1))
		meta:set_string("p2", minetest.pos_to_string(p2))
			...
			if obj:is_player() or lua_entity then
				...
				if target then
					...
					minetest.after(3, function(obj, pos, target)
						...
						if minetest.get_node(objpos).name ~= "mc_biomes:portal" then
							return
						end
						...
						local function check_and_build_portal(pos, target)
							...
							if n and n.name ~= "mc_biomes:portal" then
